...
for(i j=9;j--;)if(G[j]&((__int64)1)<<k){v p=o+v(-k
,0,-j-4);f b=p%d,c=p%p-1,q=b*b-c;if(q>0
){f s=-b-sqrt(q);if(s<t&&s>.01)t=s,n=!(
p+d*t),m=2;}}return m;}v S(v o,v d){f t
;v n;i m=T(o,d,t,n);if(!m)return v(.7,
.6,1)*pow(1-d.z,4);v h=o+d*t,l=!(v(9+R(
),9+R(),50)+h*-1),r=d+n*(n%d*-2);f b=l%
n;if(b<0||T(h,l,t,n))b=0;f p=pow(l%r*(b
>0),99);if(m&1){h=h*.2;return((i)(ceil(
h.x)+ceil(h.y))&1?v(3,1,1):v(3,3,3))*(b
*.2+.1);}return v(p,p,p)+S(h,r)*.5;}i
main(){
    cv::Mat m(512,1600,CV_8UC3);
    uchar* pix = m.data;
    v g=!v
(0,-30,0),a=!(v(0,0,1)^g)*.002,b=!(g^a
)*.002,c=(a+b)*-256+g;
    int count = 0;
    //for(i y=512;y--;)
    Concurrency::parallel_for(0,512,[&](int y)
    {
for(i x=1600;x--;){v p(13,13,13);for(i r
=64;r--;){v t=a*(R()-.5)*99+b*(R()-.5)*
99;p=S(v(7,16,8)+t,!(t*-1+(a*(R()+x)+b
*(y+R())+c)*16))*3.5+p;}
//printf("%c%c%c" ,(i)p.x,(i)p.y,(i)p.z)
 i id =(511-y)*m.step[0]+(1600-x)*3;
 pix[id] = p.z;
 pix[id +1] = p.y;
 pix[id+2] = p.x;
    ;}
